...
- **Priority discipline (beads).** `add` without `--priority` creates P2 (explicit at the adapter, not bd's implicit default). Setting `--priority 0|1` - at `add` or `update` - requires `--why "<one line>"` and refuses with `--priority <n> requires --why <one line> (P0/P1 must carry a reason)`; the reason persists as the `priority-why:` body line, echoes as `priority_why` in show/`--json`, survives `update --body`, and retires when priority rises above P1 without a replacement `--why`. Markdown has no gate and no default (unchanged); it only accepts and stores `--why`. `priorities` prints the per-level histogram plus the P0+P1 share (beads answers it through the native `priorities()` hook: one `bd list`, no deps/blocker hydration).
