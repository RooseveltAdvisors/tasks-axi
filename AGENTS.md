...
- `src/backends/beads.ts` — the only `bd` CLI adapter; its header owns the ID, hold, and dependency-reason persistence mappings, while native ready/blocked/deps selection and atomic claim stay behind the `Store` seam. Its default runner (`spawnBd`) spawns bd and streams stdout/stderr into memory with **no size cap**, surfacing bd's exit status and its reported error (stdout JSON first, then stderr - see Conventions) in the `beads <verb> failed` error - see the comment on `spawnBd` for why execFile's maxBuffer was never acceptable; `test/backends/beads-runner.test.ts` regression-guards both through a real spawned fake bd. `test/backends/beads-live.test.ts` creates a standalone temporary Git repo before `bd init`; a nested directory alone can discover and mutate the enclosing repo's Beads database.
...
- **Every `bd create` carries `--due`.** The house Beads fork enforces `due.required`, and `bd create` - unlike `bd q` - refuses rather than synthesizing a date, so an adapter that omits `--due` cannot add a task at all on a house store.
  `BEADS_DUE_LADDER` (`src/backends/beads.ts`) mirrors bd's own `bd q` ladder as relative offsets (`+1d` P0 .. `+30d` P4) handed to bd verbatim, so bd keeps owning the date arithmetic and the fleet keeps one ladder instead of two that can drift.
  Never fix this by turning the invariant off or by making bd auto-fill a due date - that disarms it for every direct `bd create` caller. Any fixture that shells out to `bd create` needs one too (`test/fixtures/synthetic-beads.ts` uses a fixed date to stay deterministic).
- **bd reports validation failures as JSON on STDOUT, not stderr.** `call` (`src/backends/beads.ts`) reads bd's `{"error": ...}` off stdout and falls back to stderr; reading stderr alone collapsed real messages into an opaque `beads create failed: exit 1` that had to be re-probed by hand.
- **Priority discipline.** `add` without `--priority` creates P2 on beads (explicit at the adapter, not bd's implicit default); markdown keeps no default. Setting `--priority 0|1` - at `add` or `update` - requires `--why "<one line>"` on beads and refuses with `--priority <n> requires --why <one line> (P0/P1 must carry a reason)`; markdown has no such gate. The mirror rule is backend-independent and lives in `parsePriorityPair` (`commands/crud.ts`): `--why` is a usage error unless the same invocation passes `--priority 0|1`, so a reason can never sit on a P2+ item. The reason persists as the `priority-why:` body line, echoes as `priority_why` in show/`--json`, survives `update --body`, and retires when priority rises above P1 without a replacement `--why`. The line is reserved: caller body text carrying it (`add`/`update` `--body`/`--body-file` and `done` `--note`) is refused with a validation error naming the rule (`assertNoManagedPriorityWhyLine` in `src/priority-why.ts`, enforced at every caller body-text seam), so caller body text is never silently lifted or lost.
